      ...
      {opponents.slice(0, 2).map((opp, idx) => {
        ...
        return (
          <div key={opp.id} style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: idx === 0 ? 0 : 6 }}>
            ...
            <span style={{ fontFamily: 'var(--font-head)', fontSize: 17, fontWeight: 700, color: 'var(--text3)', letterSpacing: '.02em', flex: 1, minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
              ({opp.seed}) {opp.name}
            </span>
            {oOwner
